...
**Deploy path:** the CI deploy job (`.github/workflows/deploy.yml`) runs `npx prisma migrate deploy` then `npx tsx prisma/seed-catalog.standalone.ts` (production-safe catalog seed — sources/courses/bundles/pathways only, **no** demo users) in one-shot containers, then recreates the web container. The catalog seed step is idempotent and non-fatal. Locally: `npm run db:seed` (full dev seed incl. demo users) or `npm run db:seed:catalog` (catalog only). Note: the repo has no init migration (base schema was bootstrapped via `db push`), so `migrate deploy` only applies incremental migrations onto an existing DB.
